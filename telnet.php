...
class Telnet {
	 ...
	 * @return array containing string Command result and array of waitPrompt's success and prompt used
	 */
	public function exec($command) {

		$this->write($command);
		$prompt_found = $this->waitPrompt();
		return array($this->getBuffer(),$prompt_found);
	}
	...
	public function login($username, $password) {
		...
		try{
			$this->setPrompt(array('login:'));
			$this->waitPrompt();
			$this->write($username);
			$this->setPrompt(array('Password:'));
			$this->waitPrompt();
			$this->write($password);
			$this->setPrompt();
			$this->waitPrompt();
	 ...
	 * @return boolean
	 */
	public function setPrompt($s = array('$')){
		$this->prompt = $s;
		return self::TELNET_OK;
	}
	...
	protected function readTo($prompt) {
		...
		do {
			...
			if (time() > $until_t) {
				$prompts='';
				foreach($prompt as $p) $prompts .= $p . " ";
				throw new Exception("Couldn't find the requested : '$prompts' within {$this->timeout} seconds");
			}
			...
			if ($c === false) {
				$prompts='';
				foreach($prompt as $p) $prompts .= $p . " ";
				throw new Exception("Couldn't find the requested : '" . $prompts . "', it was not in the data returned from server: " . $this->buffer);
			}
			...
			foreach($prompt as $p){
				if ((substr($this->buffer, strlen($this->buffer) - strlen($p))) == $p){
					return array(self::TELNET_OK,$p);
				}
			}
